...
public abstract class CuboidalMultiblock<MULTIBLOCK extends CuboidalMultiblock<MULTIBLOCK>> extends AbstractCuboidMultiblockController<MULTIBLOCK> {
    protected CuboidalMultiblock(Level world) {
        super(world);
    }
...
//        }
    ...
    public Vec3 getMiddleInteriorPlaneCoord(Direction normal, int depth, int minUCushion, int minVCushion, int maxUCushion, int maxVCushion) {
        ...
        return new Vec3((min.getX() + max.getX()) / 2F, (min.getY() + max.getY()) / 2F, (min.getZ() + max.getZ()) / 2F);
    }
