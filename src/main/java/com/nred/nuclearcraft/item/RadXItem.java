    ...
    public ItemStack finishUsingItem(ItemStack stack, Level level, LivingEntity entity) {
        if (entity instanceof Player player) {
            ...
            if (playerRads.canConsumeRadX()) {
                level.playSound(null, player.getX(), player.getY(), player.getZ(), rad_x.get(), SoundSource.PLAYERS, (float) (0.5D * radiation_sound_volumes[2]), 1F);
                onRadXConsumed(stack, level, player);
    ...
    private static void sendCooldownMessage(Level level, Player player, IEntityRads playerRads, boolean playSound) {
        if (playerRads.getRadXCooldown() > 0D && playerRads.getMessageCooldownTime() <= 0) {
            ...
            if (!level.isClientSide()) {
                ...
                player.sendSystemMessage(Component.translatable(MODID + ".message.rad_x_cooling_down", UnitHelper.applyTimeUnitShort(Math.ceil(playerRads.getRadXCooldown()), 3, 1)).withStyle(ChatFormatting.ITALIC));
            }
        }
    }

    private static void onRadXConsumed(ItemStack stack, Level level, Player player) {
        if (level.isClientSide()) {
            return;
        }
        IEntityRads playerRads = player.getCapability(CAPABILITY_ENTITY_RADS, null);
