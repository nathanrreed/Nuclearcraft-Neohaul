...
public class FissionCoolantHeaterProvider {
    public FissionCoolantHeaterProvider(RecipeOutput recipeOutput) {
        new BasicRecipeBuilder<>(new FissionCoolantHeaterRecipe(FISSION_REACTOR_MAP.get("standard_fission_coolant_heater").toStack(), SizedFluidIngredient.of(COOLANT_MAP.get("nak").still.get(), 1), SizedFluidIngredient.of(HOT_COOLANT_MAP.get("nak_hot").still.get(), 1), fission_heater_cooling_rate[0], "standard_heater")).save(recipeOutput, "_fission_coolant_heater");
        for (int i = 0; i < FISSION_HEAT_PARTS.size(); ++i) {
            new BasicRecipeBuilder<>(new FissionCoolantHeaterRecipe(FISSION_REACTOR_MAP.get(FISSION_HEAT_PARTS.get(i) + "_fission_coolant_heater").toStack(), SizedFluidIngredient.of(COOLANT_MAP.get(FISSION_HEAT_PARTS.get(i) + "_nak").still.get(), 1), SizedFluidIngredient.of(HOT_COOLANT_MAP.get(FISSION_HEAT_PARTS.get(i) + "_nak_hot").still.get(), 1), fission_heater_cooling_rate[i + 1], FISSION_HEAT_PARTS.get(i) + "_heater")).save(recipeOutput, "_heater");
        }
    }
}