...
public class SupercoolerProvider {
    public SupercoolerProvider(RecipeOutput recipeOutput) {
        new ProcessorRecipeBuilder(SupercoolerRecipe.class, 1, 1).addFluidInput(HELIUM_TAG, BUCKET_VOLUME * 8).addFluidResult(CUSTOM_FLUID_MAP.get("liquid_helium"), 25).save(qmdNotLoaded(recipeOutput));
        new ProcessorRecipeBuilder(SupercoolerRecipe.class, 0.5, 0.5).addFluidInput(NITROGEN_TAG, BUCKET_VOLUME * 8).addFluidResult(CUSTOM_FLUID_MAP.get("liquid_nitrogen"), 25).save(qmdNotLoaded(recipeOutput));
