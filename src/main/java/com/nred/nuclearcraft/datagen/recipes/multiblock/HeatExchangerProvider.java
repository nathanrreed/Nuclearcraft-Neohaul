...
public class HeatExchangerProvider {
    public HeatExchangerProvider(RecipeOutput recipeOutput) {
        ...
        new BasicRecipeBuilder<>(new HeatExchangerRecipe(Fluids.sizedIngredient(STEAM_MAP.get("high_pressure_steam"), 1), Fluids.sizedIngredient(STEAM_MAP.get("low_pressure_steam"), 4), 4D, 600, 550, false, 0, 0.0)).save(recipeOutput);
        ...
        for (String coolant : GAS_COOLANTS) {
            // Hot Gas -> Gas
            new BasicRecipeBuilder<>(new HeatExchangerRecipe(Fluids.sizedIngredient(HOT_GAS_MAP.get(coolant + "_hot"), 2), Fluids.sizedIngredient(GAS_MAP.get(coolant), 1), 1.0, HeatExchangerRecipe.RecipeHeatingType.HOT_GAS, 1000, 300, false, 0, 0.0)).save(recipeOutput);

            // Exhaust Gas -> Gas
            new BasicRecipeBuilder<>(new HeatExchangerRecipe(Fluids.sizedIngredient(HOT_GAS_MAP.get(coolant + "_exhaust"), 4), Fluids.sizedIngredient(GAS_MAP.get(coolant), 1), 0.5D, HeatExchangerRecipe.RecipeHeatingType.HOT_GAS, 800, 300, false, 0, 0.0)).save(recipeOutput);
        }

        // Hot NaK -> NaK

        new BasicRecipeBuilder<>(new HeatExchangerRecipe(Fluids.sizedIngredient(HOT_COOLANT_MAP.get("nak_hot"), 1), Fluids.sizedIngredient(COOLANT_MAP.get("nak"), 1), HeatExchangerRecipe.RecipeHeatingType.HOT_NAK, 800, 400, false, 0, 0.0)).save(recipeOutput);
        for (String coolant : COOLANTS) {
            new BasicRecipeBuilder<>(new HeatExchangerRecipe(Fluids.sizedIngredient(HOT_COOLANT_MAP.get(coolant + "_nak_hot"), 1), Fluids.sizedIngredient(COOLANT_MAP.get(coolant + "_nak"), 1), HeatExchangerRecipe.RecipeHeatingType.HOT_NAK, 800, 400, false, 0, 0.0)).save(recipeOutput);
        }
    }
}