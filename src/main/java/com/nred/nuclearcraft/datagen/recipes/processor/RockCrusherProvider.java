...
public class RockCrusherProvider {
    public RockCrusherProvider(RecipeOutput recipeOutput) {
        ...
        new ProcessorRecipeBuilder(RockCrusherRecipe.class, 1, 1).addItemInput(MATERIAL_BLOCK_MAP.get("soulless_sandstone"), 1).addItemResult(GEM_DUST_MAP.get("barite"), 80, 1).addItemResult(GEM_DUST_MAP.get("coal"), 60, 1).addItemResult(GEM_DUST_MAP.get("dysprholminite"), 40, 1).save(recipeOutput.withConditions(new ModLoadedCondition("qmd")), ncLoc("qmd"));
        new ProcessorRecipeBuilder(RockCrusherRecipe.class, 1, 1).addItemInput(MATERIAL_BLOCK_MAP.get("soulless_sandstone"), 1).addItemResult(GEM_DUST_MAP.get("barite"), 80, 1).addItemResult(GEM_DUST_MAP.get("nichromite"), 60, 1).addItemResult(GEM_DUST_MAP.get("dysprholminite"), 40, 1).save(recipeOutput.withConditions(new NotCondition(new ModLoadedCondition("qmd"))));
    }
}