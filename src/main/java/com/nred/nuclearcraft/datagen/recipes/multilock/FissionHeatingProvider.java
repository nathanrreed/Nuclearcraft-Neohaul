...
public class FissionHeatingProvider {
    public FissionHeatingProvider(RecipeOutput recipeOutput) {
        ...
        for (int i = 0; i < COOLANTS.size(); ++i) {
            new BasicRecipeBuilder<>(new FissionHeatingRecipe(SizedFluidIngredient.of(COOLANT_MAP.get(COOLANTS.get(i) + "_nak").still.get(), 1), SizedFluidIngredient.of(HOT_COOLANT_MAP.get(COOLANTS.get(i) + "_nak_hot").still.get(), 1), fission_heater_cooling_rate[i + 1], true)).save(recipeOutput);
        }
    }
}