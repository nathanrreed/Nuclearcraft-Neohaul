...
public class FluidEnricherProvider {
    public FluidEnricherProvider(RecipeOutput recipeOutput) {
        new ProcessorRecipeBuilder(FluidEnricherRecipe.class, 1, 1).addItemInput(GEM_DUST_MAP.get("boron_nitride"), 1).addFluidInput(Fluids.WATER, 1000).addFluidResult(SALT_SOLUTION.get("boron_nitride_solution"), 666).save(recipeOutput);
