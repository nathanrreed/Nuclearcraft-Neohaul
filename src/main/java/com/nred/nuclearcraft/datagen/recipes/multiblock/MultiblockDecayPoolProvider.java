...
public class MultiblockDecayPoolProvider {
    public MultiblockDecayPoolProvider(RecipeOutput recipeOutput) {
        new BasicRecipeBuilder<>(new MultiblockDecayPoolRecipe(SizedChanceFluidIngredient.of(WATER, 1), Fluids.sizedIngredient(CUSTOM_FLUID_MAP.get("preheated_water"), 1), 32)).save(recipeOutput);
    }
}