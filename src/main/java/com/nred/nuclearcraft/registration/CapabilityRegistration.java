...
public class CapabilityRegistration {
    ...
    public static void registerCapabilities(RegisterCapabilitiesEvent event) {
        ...
        for (String typeName : PROCESSOR_MAP.keySet()) {
            event.registerBlockEntity(Capabilities.ItemHandler.BLOCK, PROCESSOR_ENTITY_TYPE.get(typeName).get(), (machineEntity, direction) -> machineEntity.itemStackHandler);
            event.registerBlockEntity(Capabilities.EnergyStorage.BLOCK, PROCESSOR_ENTITY_TYPE.get(typeName).get(), (machineEntity, direction) -> machineEntity.energyHandler);
            event.registerBlockEntity(Capabilities.FluidHandler.BLOCK, PROCESSOR_ENTITY_TYPE.get(typeName).get(), (machineEntity, direction) -> machineEntity.fluidHandler);
        }
    }
}