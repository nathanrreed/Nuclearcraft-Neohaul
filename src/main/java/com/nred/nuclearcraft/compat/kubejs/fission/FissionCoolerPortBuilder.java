...
@ReturnsSelf
public class FissionCoolerPortBuilder extends BlockBuilder {
    private static final ResourceLocation MODEL = ncLoc("block/machine_overlayed");

    public FissionCoolerPortType coolerPortType;

    public FissionCoolerPortBuilder(ResourceLocation i) {
        super(i);
    ...
    protected void generateBlockModels(KubeAssetGenerator gen) {
        gen.blockModel(id.withSuffix("_on"), m -> {
            ...
            m.texture("back_overlay", "nuclearcraftneohaul:block/fission/port/cooler/out");
            ...
            m.texture("front_overlay", "nuclearcraftneohaul:block/fission/port/cooler/out");
        ...
        gen.blockModel(id.withSuffix("_off"), m -> {
            m.parent(MODEL);
            m.texture("back", baseTexture);
            m.texture("back_overlay", "nuclearcraftneohaul:block/fission/port/cooler/in");
            m.texture("bottom", "nuclearcraftneohaul:block/fission/port/top");
            m.texture("bottom_overlay", "nuclearcraftneohaul:block/block_invisible");
            m.texture("front", baseTexture);
            m.texture("front_overlay", "nuclearcraftneohaul:block/fission/port/cooler/in");
            m.texture("side", "nuclearcraftneohaul:block/fission/port/side");
            m.texture("side_overlay", "nuclearcraftneohaul:block/block_invisible");
            m.texture("top", "nuclearcraftneohaul:block/fission/port/top");
            m.texture("top_overlay", "nuclearcraftneohaul:block/block_invisible");
    ...
    public Block createObject() {
        if (coolerPortType == null) throw new RuntimeException("Missing gas cooler port data! Must be set with setCoolerPortData(fluid_id)");
        BLOCK_ENTITY_TYPES_MAP.computeIfAbsent(FISSION_ENTITY_TYPE.get("cooler_port").getId(), e -> new ArrayList<>()).add(this.id);
        return FissionPartType.CoolerPort.createBlock(coolerPortType);
    }
}