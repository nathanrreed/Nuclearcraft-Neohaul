...
import static com.nred.nuclearcraft.enumm.ButtonEnum.FLUID_OUTPUT_SLOT_SETTINGS;
import static com.nred.nuclearcraft.enumm.ButtonEnum.ITEM_OUTPUT_SLOT_SETTINGS;
...
public class CycleButton extends AbstractButton {
    ...
    public void onClick(double mouseX, double mouseY, int button) {
        ...
        if (Minecraft.getInstance().screen instanceof ProcessorScreen<?> screen) {
            //TODO send side config to server

            if (prefix.equals("tooltip.side_config.slot_setting.slot")) {
                PacketDistributor.sendToServer(new ButtonPressPayload(ITEM_OUTPUT_SLOT_SETTINGS, index, screen.getMenu().info.pos(), button == 0));
                screen.getMenu().info.itemOutputSettings().set(index, i);
            } else if (prefix.equals("tooltip.side_config.slot_setting.tank")) {
                PacketDistributor.sendToServer(new ButtonPressPayload(FLUID_OUTPUT_SLOT_SETTINGS, index, screen.getMenu().info.pos(), button == 0));
