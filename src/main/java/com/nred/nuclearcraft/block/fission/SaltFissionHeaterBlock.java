...
public class SaltFissionHeaterBlock extends GenericTooltipDeviceBlock<FissionReactor, IFissionPartType> {
    public SaltFissionHeaterBlock(MultiblockPartProperties<IFissionPartType> properties) {
        super(properties);
    }
    ...
    protected ItemInteractionResult useItemOn(ItemStack stack, BlockState state, Level level, BlockPos pos, Player player, InteractionHand hand, BlockHitResult hitResult) {
        ...
        if (!level.isClientSide() && level.getBlockEntity(pos) instanceof SaltFissionHeaterEntity heater) {
            FissionReactor reactor = heater.getMultiblockController().orElse(null);
            if (reactor != null) {
